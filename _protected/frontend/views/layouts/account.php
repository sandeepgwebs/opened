...
<head>
    ...
    <link rel="shortcut icon" href="<?= Yii::$app->request->baseUrl; ?>/favicon.ico"
          type="image/x-icon"/> <?= Html::csrfMetaTags() ?>
    <title><?= Html::encode($this->title) ?></title> <?php $this->head();
    $this->registerJs("var baseurl = " . json_encode(Yii::$app->request->baseUrl) . ";", View::POS_END); ?>    <?php if (Yii::$app->params['settings']['facebook_analatic'] != "") { ?>        <!-- Facebook Pixel Code -->
        ...
        </noscript>        <!-- End Facebook Pixel Code -->    <?php } ?></head>
<body
    class="cbp-spmenu-push"><?= Yii::$app->params['settings']['google_analatic'] ?><?= Yii::$app->params['settings']['google_adwords'] ?><?php $this->beginBody() ?>
<div id="wrapper">
    <div
        class="craftmanship-main">        <?= $this->render('header2.php') ?>    </div> <?= Alert::widget() ?>        <?= $content ?>        <?= $this->render('account_bottom.php') ?>        <?= $this->render('footer.php') ?>
</div>
<!--end full page--><?php $this->endBody() ?>
...
<script type='text/javascript'>/*<![CDATA[*/
    ...
    /*]]>*/</script>
<noscript><a href="https://www.olark.com/site/6280-566-10-8672/contact" title="Contact us" target="_blank">Questions?
