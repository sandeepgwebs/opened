...
class FeeSearch extends Fee
{
    ...
    {
        return [
            [['id', 'user_type', 'track_id', 'journal_id', 'no_of_papers', 'status', 'payment_method', 'payment_id', 'created_at', 'updated_at'], 'integer'],
            [['qualification', 'institute'], 'safe'],
            [['payment'], 'number'],
        ];
    }
    ...
    {
        $query = Fee::find()->where(['user_id'=>Yii::$app->user->identity->id]);
