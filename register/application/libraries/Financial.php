 ...
 * 
 * Financial public functions with the Excel public function names and
 * parameter order.
 * 
...
class Financial
{
	...
	*/
	public function DATEADD($datepart, $number, $date)
	{
	...
	* @return bool
	*/
	public function _is_valid_basis($basis)
	{
	...
	* @return bool
	*/
	public function _is_valid_frequency($frequency)
	{
	...
	* Use this public function to help compute payments if your accounting system
	...
	*/
	public function DAYS360($start_date, $end_date, $method = false)
	{
		if ($method) {
	...
	*/
	public function Thirty360USdayCount($startdate, $enddate)
	{
	...
	*/
	public function Thirty360USyearFraction($startdate, $enddate)
	{
	...
	* @return integer the number of days between two dates
	*/
	public function Thirty360EUdayCount($startdate, $enddate)
	{
		$d1 = getdate($startdate);
		$d2 = getdate($enddate);
		$dd1 = $d1['mday']; $mm1 = $d1['mon']; $yy1 = $d1['year'];
	...
	* @return float   the fraction of years between two dates
	*/
	public function Thirty360EUyearFraction($startdate, $enddate)
	{
	...
	* @return integer the number of days between two dates
	*/
	public function ActualActualdayCount($startdate, $enddate)
	{
	...
	* @return float   the fraction of years between two dates
	*/
	public function ActualActualyearFraction($startdate, $enddate, $refPeriodStart = null, $refPeriodEnd = null)
	{
		$t = time();
	...
	* @return integer the number of days between two dates
	*/
	public function Actual360dayCount($startdate, $enddate)
	{
		return $this->DATEDIFF('day', $startdate, $enddate);
	}
	...
	* @return float   the fraction of years between two dates
	*/
	public function Actual360yearFraction($startdate, $enddate)
	{
	...
	* @return integer the number of days between two dates
	*/
	public function Actual365dayCount($startdate, $enddate)
	{
		return $this->DATEDIFF('day', $startdate, $enddate);
	}
	...
	* @return float   the fraction of years between two dates
	*/
	public function Actual365yearFraction($startdate, $enddate)
	{
	...
	*/
	public function DOLLARDE($fractional_dollar, $fraction)
	{
	...
	*/
	public function DOLLARFR($decimal_dollar, $fraction)
	{
		$fraction = intval($fraction);
	...
	*/
	public function SYD($cost, $salvage, $life, $per)
	{
		$life = intval($life);
		$per  = intval($per);
	...
	* @return float
	*/
	public function _ScGetGDA($fWert, $fRest, $fDauer,$fPeriode, $fFaktor)
	{
	...
	* @return float
	*/
	public function _ScInterVDB($cost, $salvage, $life, $life1, $period, $factor)
	{
		$fVdb       = 0;
		$fIntEnd    = ceil($period);
		$nLoopEnd   = $fIntEnd;
	...
	*/
	public function VDB($cost, $salvage, $life, $start_period, $end_period, $factor = 2.0, $no_switch = false)
	{
		// pre-validations
		if (($start_period < 0)
	...
	*/
	public function _calculate_pvif ($rate, $nper)
	{
	...
	*/
	public function _calculate_fvifa ($rate, $nper)
	{
	...
	public function _calculate_interest_part ($pv, $pmt, $rate, $per)
	{
		return -($pv * pow(1 + $rate, $per) * $rate +
			$pmt * (pow(1 + $rate, $per) - 1));
	}
	
	public function _calculate_pmt ($rate, $nper, $pv, $fv, $type)
	{
	...
	*/
	public function PV($rate, $nper, $pmt, $fv = 0.0, $type = 0)
	{
		// Calculate the PVIF and FVIFA
		$pvif  = $this->_calculate_pvif ($rate, $nper);
		$fvifa = $this->_calculate_fvifa ($rate, $nper);
	 ...
	 * 
	 * For a more complete description of the arguments in FV, see the PV public function.
	 * 
	 ...
	 * 
	 **/
	public function FV($rate, $nper, $pmt, $pv = 0.0, $type = 0)
	{
		$pvif = $this->_calculate_pvif ($rate, $nper);
		$fvifa = $this->_calculate_fvifa ($rate, $nper);
	 ...
	 * 
	 */
	public function FVSCHEDULE($principal, $schedule)
	{
		$n = count($schedule);
		for ($i = 0; $i < $n; $i++)
	 ...
	 * 
	 * For a more complete description of the arguments in PMT, see the PV public function.
	 * 
	 ...
	 * 
	 */
	public function PMT($rate, $nper, $pv, $fv = 0.0, $type = 0)
	{
	 ...
	 * 
	 * For a more complete description of the arguments in IPMT, see the PV public function.
	 * 
	 */
	public function IPMT($rate, $per, $nper, $pv, $fv = 0.0, $type = 0)
	{
		if (($per < 1) || ($per >= ($nper + 1)))
			return null;
		else {
	 ...
	 * 
	 */
	public function PPMT($rate, $per, $nper, $pv, $fv = 0.0, $type = 0)
	{
		if (($per < 1) || ($per >= ($nper + 1)))
			return null;
		else {
	 ...
	 * 
	 */
	public function NPER($rate, $pmt, $pv, $fv = 0.0, $type = 0)
	{
		if (($rate == 0) && ($pmt != 0))
			$nper = (-($fv + $pv) / $pmt);
		elseif ($rate <= 0.0)
	 ...
	 * 
	 */
	public function EFFECT($nominal_rate, $npery)
	{
		$npery = intval($npery);
	 ...
	 * 
	 **/
	public function NOMINAL($effect_rate, $npery)
	{
		$npery = intval($npery);
	 ...
	 * 
	 */
	public function DISC($settlement, $maturity, $pr, $redemption, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
	 ...
	 * 
	 */
	public function RECEIVED($settlement, $maturity, $investment, $discount, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
		if (!$this->_is_valid_basis($basis)) return null;
	 ...
	 * 
	 */
	public function INTRATE($settlement, $maturity, $investment, $redemption, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
		if (!$this->_is_valid_basis($basis)) return null;
	 ...
	 * for a series of cash flows that is periodic, use the NPV public function.
	 * 
	 *        n   /                values(i)               \
	 * NPV = SUM | ---------------------------------------- |
	 ...
	 * 
	 */
	public function MIRR($values, $finance_rate, $reinvert_rate)
	{
		$n = count($values);
	 ...
	 * of return for a series of periodic cash flows, use the IRR public function.
	 * 
	 ...
	 * Use this public function to filter a set of values. For example, by summing several DELTA public functions
	 * you calculate the count of equal pairs. This public function is also known as the Kronecker Delta public function.
	 */
	public function DELTA($number1, $number2 = 0)
	{
	 ...
	 *
	 */
/*	public function YIELD($settlement, $maturity, $rate, $pr, $redemption, $frequency, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
		if (!$this->_is_valid_basis($basis)) return null;
	 ...
	 */
	public function PRICEDISC($settlement, $maturity, $discount, $redemption, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
		if (!$this->_is_valid_basis($basis)) return null;
	 ...
	 **/
	public function YIELDDISC($settlement, $maturity, $pr, $redemption, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
		if (!$this->_is_valid_basis($basis)) return null;
		if (($pr <= 0) || ($redemption <= 0)) return null;
		if ($settlement >= $maturity) return null;
	 ...
	 *
	 */
	public function COUPNUM($settlement, $maturity, $frequency, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
		if (!$this->_is_valid_basis($basis)) return null;
		if (!$this->_is_valid_frequency($frequency)) return null;
		if ($settlement >= $maturity) return null;
	 ...
	 *
	 */
	public function COUPDAYBS($settlement, $maturity, $frequency, $basis = FINANCIAL_BASIS_MSRB_30_360)
	{
		if (!$this->_is_valid_basis($basis)) return null;
		if (!$this->_is_valid_frequency($frequency)) return null;
		if ($settlement >= $maturity) return null;
